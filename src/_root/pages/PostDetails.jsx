    ...
    <div className="w-auto min-h-screen h-fit ml-[300px] flex justify-center max-[1590px]:ml-[120px] max-[1080px]:ml-0 max-[1080px]:mt-[60px]">
      <div className="flex flex-col w-[850px] gap-5 my-[30px] max-[1080px]:mb-[100px] max-[900px]:w-[650px] max-[700px]:w-[450px] max-[500px]:w-[300px]">
        <div className="flex items-center gap-3">
          <img src={"https://localhost:7018/Imgs/" + postDetails?.userProfileImg}  className="rounded-[50%] w-[45px] h-[45px] object-cover" />
          <div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {tags && tags.map((tag, index) => (
              <p key={index} className="p-1 px-2 bg-colors-color3 rounded-[8px]">#{tag.trim()}</p>
            ))}
          </div>
        </div>
            ...
            <div key={index} className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[12px]">{comment.createdAt.split('T')[0]}</p>
                </div>
              </div>
        ...
        <div className="bg-colors-color1 p-2 px-4 flex rounded-[8px] ">
          <input name="description" onChange={formik.handleChange}type="text" className="outline-none bg-colors-color1 w-[100%]" placeholder="Add a comment ..." />
          <button onClick={formik.handleSubmit} className="p-1 px-3 rounded-[8px] bg-colors-color3">Post</button>
        </div>
      </div>
    </div>
  );
